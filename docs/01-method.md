...
A league's own feed or an operator's own document outranks a strategy blog. No
operator scoring page could be retrieved (L-02), so *nothing here is
`confirmed_by_operator`* - and the docs say that on every page rather than once.
...
**459 tests** across 12 modules, all passing, run with
...
| `test_engines` | 42 | position dispatch, no-fabrication, win probability, bands |
...
- **fanduel.com is geo-blocked**; both operators keep their scoring pages inside
  their logged-in apps.
